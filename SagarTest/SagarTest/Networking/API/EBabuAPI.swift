...
let EBabuAPIProvider = MoyaProvider<EBabuAPI>(plugins: [NetworkLoggerPlugin(verbose: true, responseDataFormatter: JSONResponseDataFormatter)])
enum EBabuAPI {
...
extension EBabuAPI: TargetType {
    ...
    var path: String {
        switch self {
        case .batman:
            return "?s=Batman&page=1&apikey=eeefc96f"
        }
    }
